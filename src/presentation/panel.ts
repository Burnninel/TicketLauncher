...
export class Panel {
	...
	showCompany(companyName: string, cnpj: string): void {
		this._setCnpj(cnpj);
		this._lockCnpj();
		this._messageInput.value = MESSAGES.defaults.ticketDescription;
		this._status.showForm(companyName, cnpj);
	...
	showTicketCreated(ticketNumber: string): void {
		this._status.showSuccess(MESSAGES.success.ticketNumber(ticketNumber));
	}
